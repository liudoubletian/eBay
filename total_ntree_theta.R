...
for (h in 1: 6){ 
  ...
  for ( m in  1:lotime){
    ...
    for(i in 1:N.sam){
      ...
      set.seed(i*m*(seed+1))
      ntree_table[i+N.sam,] <- simPop(J=1, K=p, n=parent2, pi = case_pi, theta = the_eff_nt)$data
    }
...
write.table(new_tree_power_samll_effect,"/export/home/yangsheng/double/emp_norm/algtotal_ntree_theta.txt")
