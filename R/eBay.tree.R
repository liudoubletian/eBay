...
#' ebay.res <- eBay_tree(otu.table,tree=tree,group=group,test.method="t",cutf=0.05)
#' @export
...
eBay_tree=function(otu.data,tree,group,test.method,cutf){
  ...
  detec_result=detec_otus(p.val=p.vector,tree_table=tree_table,tree=pru_tree,group,test.method,cutf)
